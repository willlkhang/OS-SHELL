run: main.o signal_processor.o parser.o utilities.o separators.o tokenizer.o builtins_executor.o command_executor.o tokenizer_utilities.o command_setup.o history_process.o wildcard_handler.o interactive_input.o
	gcc -Wall main.o signal_processor.o parser.o utilities.o separators.o tokenizer.o builtins_executor.o command_executor.o tokenizer_utilities.o command_setup.o history_process.o wildcard_handler.o interactive_input.o -o run

main.o: main.c signal_processor.h command.h parser.h utilities.h separators.h tokenizer.h builtins_executor.h command_executor.h tokenizer_utilities.h command_setup.h history.h history_process.h interactive_input.h
	gcc -Wall -c main.c

signal_processor.o: signal_processor.c signal_processor.h
	gcc -Wall -c signal_processor.c signal_processor.h

parser.o: parser.c parser.h
	gcc -Wall -c parser.c parser.h

utilities.o: utilities.c utilities.h
	gcc -Wall -c utilities.c utilities.h

separators.o: separators.c separators.h
	gcc -Wall -c separators.c separators.h

tokenizer.o: tokenizer.c tokenizer.h
	gcc -Wall -c tokenizer.c tokenizer.h

builtins_executor.o: builtins_executor.c builtins_executor.h
	gcc -Wall -c builtins_executor.c builtins_executor.h

command_executor.o: command_executor.c command_executor.h
	gcc -Wall -c command_executor.c command_executor.h

tokenizer_utilities.o: tokenizer_utilities.c tokenizer_utilities.h
	gcc -Wall -c tokenizer_utilities.c tokenizer_utilities.h

command_setup.o: command_setup.c command_setup.h
	gcc -Wall -c command_setup.c command_setup.h

history_process.o: history_process.c history_process.h
	gcc -Wall -c history_process.c history_process.h

wildcard_handler.o: wildcard_handler.c wildcard_handler.h
	gcc -Wall -c wildcard_handler.c wildcard_handler.h

interactive_input.o: interactive_input.c interactive_input.h
	gcc -Wall -c interactive_input.c interactive_input.h

cls:
	rm -f *.o *.gch

clean:
	rm -f *.o *.gch run
